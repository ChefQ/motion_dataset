...
if arg.all == True:
    ...
    ('./predictions/RFT-sentence_embeddings-supppredictions.csv', './predictions/RFT-sentence_embeddings-oppopredictions.csv'): {'includeLogOdds': True, 'changeDenyProb': True}, 
    ...
    ('./predictions/LLM-bert-base-uncased-supppredictions.csv', './predictions/LLM-bert-base-uncased-oppopredictions.csv'): {'includeLogOdds': True, 'changeDenyProb': False}
    }
    ...
    if arg.type == 'fuse':
        ...
        for (path1, path2), args in pairs.items():
            ...
            grouped1 = derive_fuse(df1).drop(columns=['log_odds' ])
            grouped1 = grouped1[['folder_id' , 'predict','newScore', 'truth']]
            grouped1 = grouped1.rename(columns={'newScore': 'score'})

            grouped1.to_csv(path1.replace('/predictions' , '/predictions/fuse').replace('.csv', '_fuse.csv'), index=False)

            df2 = add_folderId(path2, **args)
            grouped2 = derive_fuse(df2).drop(columns=['log_odds' ])
            grouped2 = grouped2[['folder_id' , 'predict','newScore', 'truth']]
            grouped2 = grouped2.rename(columns={'newScore': 'score'})
            grouped2.to_csv(path2.replace('/predictions' , '/predictions/fuse').replace('.csv', '_fuse.csv'), index=False)

            df = pd.concat([df1, df2])
            grouped = derive_fuse(df)
            grouped = derive_fuse(df).drop(columns=['log_odds' ])
            grouped = grouped[['folder_id' , 'predict','newScore', 'truth']]
            grouped = grouped.rename(columns={'newScore': 'score'})
            file_type = 'supp' if 'supp' in path1 else 'oppo'
            grouped.to_csv(path1.replace('/predictions' , '/predictions/fuse').replace(file_type, "both").replace('.csv', '_fuse.csv'), index=False)
        ...
        for (path1, path2), args in pairs.items():
            ...
            df1 = add_folderId(path1, **args)
            new_df1 = derive_first(df1).drop(columns=['score', 'log_odds', 'brief'])
            new_df1 = new_df1[['folder_id' , 'predict','newScore', 'truth']]
            new_df1 = new_df1.rename(columns={'newScore': 'score'})
            new_df1.to_csv(path1.replace('/predictions' , '/predictions/first').replace('.csv', '_first.csv'), index=False)

            df2 = add_folderId(path2, **args)
            new_df2 = derive_first(df2)
            new_df2 = new_df2.drop(columns=['score', 'log_odds', 'brief'])
            new_df2 = new_df2[['folder_id' , 'predict','newScore', 'truth']]
            new_df2 = new_df2.rename(columns={'newScore': 'score'})
            new_df2.to_csv(path2.replace('/predictions' , '/predictions/first').replace('.csv', '_first.csv'), index=False)
    ...
    if arg.type == 'fuse':
        ...
        grouped = grouped[['folder_id' , 'predict','newScore', 'truth']]
        grouped = grouped.rename(columns={'newScore': 'score'})
        grouped.to_csv(arg.path.replace('/predictions' , '/predictions/fuse').replace('.csv', '_fuse.csv'), index = False)
        print('done')
        ...
        new_df = new_df.drop(columns=['score', 'log_odds', 'brief'])
        new_df = new_df[['folder_id' , 'predict','newScore', 'truth']]
        new_df = new_df.rename(columns={'newScore': 'score'})
        new_df.to_csv(arg.path.replace('/predictions' , '/predictions/first').replace('.csv', '_first.csv'), index=False)
        print('done')
